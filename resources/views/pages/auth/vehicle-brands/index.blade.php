...
new class extends Component
{
    ...
    public function delete($id)
    {
        ...
        $this->dispatch('toast', message: 'Marca excluída com sucesso', type: 'success');
    }

    public function getVehicleBrandsProperty()
    {
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center h-auto sm:h-16 py-3 sm:py-0 gap-3 sm:gap-0">
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                    ...
                    <span class="text-sm sm:text-base text-gray-700 dark:text-gray-300">Marcas</span>
                </div>
                <a href="{{ route('vehicleBrands.create') }}" class="text-center px-4 py-2 text-sm sm:text-base bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition" wire:navigate>
                    Nova Marca
                </a>
    ...
    <main class="max-w-7xl mx-auto py-4 sm:py-6 px-4 sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">Marcas</h1>
                <div class="w-full sm:w-auto">
                    <label for="filterSearch" class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">nome</label>
                    <input
                        id="filterSearch"
                        type="text"
            ...
            @if($this->vehicleBrands->count() > 0)
                <div class="hidden sm:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($this->vehicleBrands as $vehicleBrand)
                                <tr>
                                    ...
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {{ $vehicleBrand->created_at->format('d/m/Y') }}
                                    </td>
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a
                                            href="{{ route('vehicleBrands.edit', $vehicleBrand->id) }}"
                                        ...
                                        <button
                                            wire:click="delete({{ $vehicleBrand->id }})"
                                            wire:confirm="Tem certeza que deseja excluir esta marca?"
                ...
                <div class="sm:hidden divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach($this->vehicleBrands as $vehicleBrand)
                        <div class="p-4">
                            <div class="font-medium text-gray-900 dark:text-gray-100 mb-2">{{ $vehicleBrand->title }}</div>
                            <div class="text-sm text-gray-500 dark:text-gray-400 mb-3">Criado: {{ $vehicleBrand->created_at->format('d/m/Y') }}</div>
                ...
                <div class="px-4 sm:px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                    {{ $this->vehicleBrands->links() }}
                </div>
            @else
                <div class="px-4 sm:px-6 py-8 sm:py-12 text-center">
                <p class="text-sm sm:text-base text-gray-500 dark:text-gray-400 mb-4">Nenhuma marca encontrada.</p>
                    <a
                        href="{{ route('vehicleBrands.create') }}"
